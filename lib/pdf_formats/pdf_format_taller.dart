...
pw.Document buildTallerPDF({
  ...
  String joinList(List<String> list) =>
      list.isEmpty ? '' : list.map((e) => '* $e').join('\n');
  ...
  pdf.addPage(
    pw.Page(
      ...
      build: (context) => pw.Column(
        ...
        children: [
          pw.Center(
            child: pw.Text('Planeación Taller Crítico: $titulo',
          ...
          pw.Table.fromTextArray(
            ...
            data: [
              ['1. Situación inicial', momentos['situacion_inicial'] ?? ''],
              ['2. Organización de las acciones', momentos['organizacion_acciones'] ?? ''],
              ['3. Puesta en marcha', momentos['puesta_marcha'] ?? ''],
              ['4. Valoramos lo aprendido', momentos['valoramos_aprendido'] ?? ''],
            ],
            headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold),
            cellAlignment: pw.Alignment.topLeft,
            columnWidths: {
              0: const pw.FlexColumnWidth(2), // Momentos más pequeño
              1: const pw.FlexColumnWidth(6), // Descripción más grande
            },
          ),
          pw.SizedBox(height: 8),
          pw.Table.fromTextArray(
